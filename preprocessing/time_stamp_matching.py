...
folder = "validation" #"training"  or "testing" or "validation"
...
for exp in experiments:
    # Read images
    images = [os.path.basename(x) for x in glob.glob(exp + "/center/*.jpg")]
    im_stamps = []
    for im in images:
    ...
    new_fname = exp + "/sync_steering.csv"
    sync_steer_df = pd.DataFrame(sync_steer, columns=['filename','angle','torque','speed','lat','long','alt'])
    sync_steer_df.to_csv(new_fname, sep=',', index=False)
