...
def main(FLAGS):
    ...
    if device.type == 'cuda':
        ...
        if not os.path.exists(FLAGS.experiment_rootdir_het):
            os.makedirs(FLAGS.experiment_rootdir_het)
        # Hyperparameters
        ...
        # Output dimension
        output_dim = 1
        model = resnet8_MCDO_ale(img_channels, crop_img_height, crop_img_width, output_dim).to(device)
        model_ckpt = os.path.join(FLAGS.experiment_rootdir,'resnet8_MCDO_ale.pt')
        model.load_state_dict(torch.load(model_ckpt))
    
    	# Get predictions and ground truth
        _, pred_steerings_mean, aleatoric_variances, real_steerings, total_variances = \
        utils.compute_predictions_and_gt_het(model, test_loader, device, FLAGS)
        ...
        if FLAGS.is_MCDO: 
            parsed_exp_path = os.path.join(FLAGS.experiment_rootdir_het, "MCDO_het_T{}".format(FLAGS.T))
        else: 
            parsed_exp_path = os.path.join(FLAGS.experiment_rootdir_het, "standard_het")
