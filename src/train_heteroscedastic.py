...
from pytorchtools import EarlyStopping, visualize_loss, compute_l2_reg, het_loss

from model_zoo.models_resnet8 import resnet8_MCDO_ale
...
def train_model_ale(model, num_epochs, learning_rate, train_loader, valid_loader, 
                patience, model_name):
    ...
    for epoch in range(1, num_epochs+1):
        ...
        # TRAIN the model #
        ###################
        print("Current training epoch: ", epoch)
...
#            targets = targets.view(-1,1)
            ...
            out_mean = outputs['mean'].view(-1)
            out_log_var = outputs['log_var'].view(-1)
            # Calculate loss
            l2_reg = compute_l2_reg(model,model_name, FLAGS)
            loss = my_loss(out_mean,out_log_var,targets,epoch-1, FLAGS) + l2_reg
            # Backward pass
            loss.backward()
            # Update weights
            optimizer.step()
        ...
        for images, targets in valid_loader:
            ...
            out_log_var = outputs['log_var'].view(-1)
            # Calculate loss
            loss = my_loss(out_mean,out_log_var,targets,epoch-1, FLAGS)
...
def main(FLAGS):
    ...
    if device.type == 'cuda':
        ...
        print("Training model: resnet8_MCDO_ale")
        # Select model to train
        # Train MCDO resnet8
        model_name='resnet8_MCDO_ale'
        model_MCDO_ale = resnet8_MCDO_ale(img_channels, crop_img_height, crop_img_width, output_dim).to(device)
        model_MCDO_ale, train_loss_MCDO_ale, valid_loss_MCDO_ale = train_model_ale(model_MCDO_ale, num_epochs, learning_rate, train_loader, valid_loader, patience, model_name)
        visualize_loss(train_loss_MCDO_ale,valid_loss_MCDO_ale,model_name) # Store visualization of loss trends
        # Storing trained model
        ckpt_name = os.path.join(FLAGS.experiment_rootdir , model_name + '.pt')
        torch.save(model_MCDO_ale.state_dict(), ckpt_name)
